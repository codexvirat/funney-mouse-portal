...
const ZERO = { memberDiscount: 0, happyHourDiscount: 0 };
...
export function useAutoDiscount(phone, items) {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      try {
        ...
        if (!cancelled) setAutoDiscount({ memberDiscount: data.memberDiscount || 0, happyHourDiscount: data.happyHourDiscount || 0 });
